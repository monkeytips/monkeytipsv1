...
bool pool_wallet::confirmTransaction(TransferCommand cmd, bool multiAddress) {
  ...
  /* 10 tips = 0.1 WTIP */
  if (cmd.fee == 10) {
    feeString = "0.1 WTIP (minimum)";
  } else {
    feeString = m_currency.formatAmount(cmd.fee) + " WTIP";
  }
  ...
  if (!multiAddress) {
    std::cout << "You are sending " << m_currency.formatAmount(cmd.dsts[0].amount)
              << " WTIP, with a fee of " << feeString << std::endl
    ...
    for (auto destination : cmd.dsts) {
      std::cout << "You are sending " << m_currency.formatAmount(destination.amount)
                << " WTIP" << std::endl << "FROM: " << walletName << std::endl
...
bool pool_wallet::print_outputs_from_transaction(const std::vector<std::string>& args)
{
  ...
  for (size_t i = 0; i < ourTransaction.outputs.size(); ++i)
  {
    ...
    if (targetPubKey.key == outputPublicKey)
    {
      ...
      std::string wtip = m_currency.formatAmount(amount);

      sum += amount;

      found = true;

      logger(INFO, GREEN) << "The transaction output of " << wtip << " WTIP belongs to you!";
    }
  }
  ...
  else
  {
    ...
    logger(INFO, GREEN) << "Outputs totalling " << wtip << " WTIP were sent to your wallet!";
  }
...
int main(int argc, char* argv[]) {
  ...
  std::cout << "Worktips v" << PROJECT_VERSION << " Poolwallet" << std::endl;

  std::cout << "Please note that usage of simplewallet/poolwallet has been "
            << "deprecated for pool usage." << std::endl
            << "If you are using worktips-pool, you can trivially transfer to "
            << "walletd by following these instructions:" << std::endl
            << "https://github.com/vordas/worktips-pool/pull/5" << std::endl;
